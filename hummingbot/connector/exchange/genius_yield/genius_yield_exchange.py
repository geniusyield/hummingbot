...
from hummingbot.connector.exchange.genius_yield.genius_yield_api_order_book_data_source import BinanceAPIOrderBookDataSource
from hummingbot.connector.exchange.genius_yield.genius_yield_api_user_stream_data_source import BinanceAPIUserStreamDataSource
from hummingbot.connector.exchange.genius_yield.genius_yield_auth import BinanceAuth
...
class BinanceExchange(ExchangePyBase):
    ...
    def name(self) -> str:
        if self._domain == "com":
            ...
            return f"genius_yield_{self._domain}"
    ...
    async def _place_order(self,
        ...
        type_str = BinanceExchange.genius_yield_order_type(order_type)
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://github.com/genius_yield-exchange/genius_yield-official-api-docs/blob/master/user-data-stream.md
                # As per the order update section in GeniusYield the ID of the order being canceled is under the "C" key
    ...
    async def _update_order_fills_from_trades(self):
        ...
        in case GeniusYield's user stream events are not working.
        NOTE: It is not required to copy this functionality in other connectors.
        This is separated from _update_order_status which only updates the order status without producing filled
        events, since GeniusYield's get order endpoint does not return trade IDs.
        ...
        if (long_interval_current_tick > long_interval_last_tick
            ...
            query_time = int(self._last_trades_poll_genius_yield_timestamp * 1e3)
            self._last_trades_poll_genius_yield_timestamp = self._time_synchronizer.time()
